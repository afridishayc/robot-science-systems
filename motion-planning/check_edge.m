...
function in_collision = check_edge(robot, q_start, q_end, link_radius, sphere_centers, sphere_radii, resolution)
    ...
    n = length(ticks);
    % generating way points(also configurations) between the start configuration and the goal
    % configuration.
    ...
    % for each way point
    for i = 1:n
        % if a waypoint configuration is in collision with an obstacle,
        % then the path between the start and teh end configurations is in
        % collision
