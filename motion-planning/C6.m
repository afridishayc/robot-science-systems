...
function num_collisions = C6(robot, obstacles, q_path)
    ...
    for current_config = q_path'
        if isempty(prev_config)
            ...
            prev_pivots = [prev_pivot1 prev_pivot2];
        end
        ...
        for obstacle = obstacles
            ...
            if intersection_areas_1 > 0 | intersection_areas_2 > 0
                ...
                prev_piv2 = prev_pivots(:, 2);
                plot(prev_piv2(1), prev_piv2(2), 'k.', 'MarkerSize', 10);
                plot(current_pivot1(1), current_pivot1(2), 'k.', 'MarkerSize', 10);
                plot(current_pivot2(1), current_pivot2(2), 'k.', 'MarkerSize', 10);
            end
        end
        prev_polys = [current_poly1 current_poly2];
        prev_pivots = [current_pivot1 current_pivot2];
    end
end